...
class TriggerReminderRequest(BaseModel):
    ...
    """ # noqa: E501
    channels: Optional[List[StrictStr]] = Field(default=None, description="Bu çağrıda kullanılacak kanal(lar). Yollanmazsa default `[\"sms\",\"email\"]` ile her iki kanalda da gönderilir (parti `send_sms`/`send_email` toggle'ı + iletişim alanı + demand global toggle uygunluğuna göre). ", json_schema_extra={"examples": [["sms"]]})
    force: Optional[StrictBool] = Field(default=False, description="`true` ise 5 dk anti-spam penceresini override eder (son hatırlatma 5 dk içinde gönderilmiş olsa bile gönderir). Production akışlarında yanlışlıkla spam atmamak için sadece kasıtlı admin operasyonlarında kullanın. ")
    __properties: ClassVar[List[str]] = ["channels", "force"]
