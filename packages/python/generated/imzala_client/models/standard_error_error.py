...
class StandardErrorError(BaseModel):
    """
    StandardErrorError
    """ # noqa: E501
    code: StrictStr = Field(json_schema_extra={"examples": ["DEMAND_NOT_FOUND"]})
    message: StrictStr = Field(json_schema_extra={"examples": ["Demand not found"]})
    retry_after_seconds: Optional[StrictInt] = Field(default=None, description="Sadece RATE_LIMITED'de doludur", json_schema_extra={"examples": [247]})
    __properties: ClassVar[List[str]] = ["code", "message", "retry_after_seconds"]

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['INVALID_CHANNELS', 'DEMAND_NOT_FOUND', 'ALREADY_COMPLETED', 'RATE_LIMITED', 'MAX_SMS_REMINDERS_REACHED', 'UNAUTHORIZED', 'INTERNAL_ERROR']):
            raise ValueError("must be one of enum values ('INVALID_CHANNELS', 'DEMAND_NOT_FOUND', 'ALREADY_COMPLETED', 'RATE_LIMITED', 'MAX_SMS_REMINDERS_REACHED', 'UNAUTHORIZED', 'INTERNAL_ERROR')")
        return value
