...
class ReminderSettings(BaseModel):
    ...
    """ # noqa: E501
    enabled: Optional[StrictBool] = Field(default=True, description="false → hiç hatırlatma gönderilmez (cron schedule edilmez)")
    intervals_hours: Optional[List[Annotated[int, Field(strict=True, ge=1)]]] = Field(default=None, description="Sözleşmenin oluşturulduğu andan itibaren saat cinsinden hatırlatma aralıkları. Örn. `[24, 72, 168]` → 24 saat sonra, 3 gün sonra ve 7 gün sonra. `max_reminders` ile limit edilir. ", json_schema_extra={"examples": [[24, 72]]})
    max_reminders: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=1, description="Bir parti için maksimum gönderilecek hatırlatma sayısı", json_schema_extra={"examples": [2]})
    channels: Optional[List[StrictStr]] = Field(default=None, description="Hatırlatma gönderim kanalı. Birden fazla seçilebilir (`[\"email\",\"sms\"]`). SMS kanalı için partinin `phone` alanı dolu ve `send_sms: true` olmalı; email kanalı için `email` dolu ve `send_email: true` olmalı, ayrıca demand'in `send_sms_notifications` / `send_email_notifications` global toggle'ı açık olmalı. ", json_schema_extra={"examples": [["email", "sms"]]})
    __properties: ClassVar[List[str]] = ["enabled", "intervals_hours", "max_reminders", "channels"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "intervals_hours": obj.get("intervals_hours"),
            "max_reminders": obj.get("max_reminders") if obj.get("max_reminders") is not None else 1,
