...
class UpsertItemsRequest(BaseModel):
    ...
    """ # noqa: E501
    items: List[PageItem]
    page_ids: Optional[List[StrictInt]] = Field(default=None, description="**Opsiyonel.** Verilirse sadece bu sayfaların item'ları replace edilir; diğer sayfalardaki item'lar korunur. Body'deki `items[].page_id` değerleri bu listede olmalıdır. Omitted ise tüm sayfaların item'ları replace edilir. ")
    __properties: ClassVar[List[str]] = ["items", "page_ids"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "items": [PageItem.from_dict(_item) for _item in obj["items"]] if obj.get("items") is not None else None,
